...
*中文版本由[ryanhe312](github.com/ryanhe312)翻译，并使用[DeepL](deepl.com)辅助翻译，如有错误，敬请指正。*
...
-   [科学提升模型性能](#科学提升模型性能)
    ...
    -   [选择下次实验的目标](#选择下次实验的目标)
    -   [设计下次实验](#设计下次实验)
    -   [确定是否需要调整训练流程或超参数](#确定是否需要调整训练流程或超参数)
    -   [搜索结束之后](#搜索结束之后)
-   [决定每次训练的步数](#决定每次训练的步数)
    -   [没有算力限制时如何决定训练时长](#没有算力限制时如何决定训练时长)
    -   [存在算力限制时如何决定训练时长](#存在算力限制时如何决定训练时长)
-   [训练流程其他注意事项](#训练流程其他注意事项)
    -   [优化输入管道](#优化输入管道)
    -   [评价模型性能](#评价模型性能)
    -   [保存检查点和选择最佳检查点](#保存检查点和选择最佳检查点)
    -   [设置实验跟踪](#设置实验跟踪)
    -   [批归一化实现](#批归一化实现)
    -   [分布式训练注意事项](#分布式训练注意事项)
-   [常见问题与解答](#常见问题与解答)
...
本手册是为那些对**最大化深度学习模型的性能感兴趣**的工程师和研究人员（包括个人和团队）准备的。我们假设读者 机器学习和深度学习概念的基本知识。

我们的重点是**超参数调整的过程**。我们也会涉及到深度学习训练的其他方面，如训练流程实现和优化。但我们不会完整地探讨这些方面。

我们假设机器学习问题是一个监督学习问题或一些看起来很类似的问题（比如自监督学习）。也就是说，本手册中的一些规定也可能适用于其他类型的问题。

## 为什么需要一本“炼丹手册”

目前，要想让深度神经网络在实践中取得好的效果，通常需要付出惊人的努力和消耗大量的猜想。更糟糕的是，人们调参并取得优秀结果的“炼丹配方”通常很少被记录下来。论文研究者们掩盖了得到最终结果的过程，以呈现一个干净的故事。然而从事商业问题的机器学习工程师则很少有时间回头概括他们的过程。并且教科书倾向于避开实践指导，即使其作者有充足的实践经验来提供有用的建议，教科书里还是会优先考虑讲授一些基本原则。

所以，在筹划这份文档的时候，我们找不到任何全面的文档来实际解释**如何用深度学习获得好的结果**。相反，我们只能在博客文章和社交媒体上发现一些碎片的建议，从研究论文的附录中窥视到的技巧，偶尔关于某个特定项目或流程的案例研究，以及大量的混乱的描述。

虽然深度学习专家和不太熟练的从业者们使用表面上类似的方法，但是所取得的结果之间通常存在着巨大的差距。同时，这些专家也欣然承认他们所做的一些事情可能并没有充分的理由。随着深度学习的成熟并对世界产生更大的影响，整个社区需要更多的资源来产出有用的“炼丹配方”，包括所有的实际细节，因为这样的“炼丹配方”对后来人获得好的结果是非常关键的。

我们是一个由五名研究人员和工程师组成的团队，他们已经在深度学习领域工作了很多年，其中一些人早在2006年就开始工作了。我们已经将深度学习应用于从语音识别到天文学的各种问题，并在此过程中学到了很多。这份文件是从我们自己训练神经网络、教导新的机器学习工程师以及为我们的同事提供深度学习实践建议的经验中产生的。

虽然我们很高兴看到深度学习从一个由少数学术实验室实践的机器学习方法变成了为数十亿人使用的产品提供动力的技术，但作为一门工程学科，深度学习仍然处于起步阶段，我们希望这份文件能够激励其他人，和我们一起来帮助系统化该领域的实验协议。

这份文件是在我们试图理清我们自己的深度学习方法时产生的，因此它代表了作者在写作时的主观经验，而不是任何形式的客观真理。我们自己在超参数调整方面的挣扎使其成为我们指导的重点，但我们也涵盖了我们在工作中遇到的其他重要问题（或看到出错）。

我们的目的是让这项工作成为一份活的文档，随着我们信念的改变而不断成长和发展。例如，关于调试和减少训练失败的材料，我们两年前是不可能写的，因为它是基于最近的结果和正在进行的调查。不可避免地，为了考虑到新的结果和改进的工作流程，我们的一些建议需要不断更新。

我们并不懂得**最佳**的深度学习配方，因为在整个社区开始书写和辩论不同的训练流程之前，我们大概是找不到的。所以，我们鼓励那些发现我们的建议有问题的读者提出其他建议，并提供令人信服的证据，以便我们能够更新调参手册。

我们也希望看到可能有不同建议的其他指南和游戏手册，这样我们就可以作为一个社区努力实现最好的训练流程。最后，任何标有🤖表情符号的部分都是我们想做更多研究的地方。只有在尝试写完这本游戏手册之后，我们才完全明白，在深度学习从业者的工作流程中可以找到多少有趣的和被忽视的研究问题。

## 项目起步指南

其实，我们在调参过程中作出的许多决定在最开始就已经确定了，我们只会在情况发生变化时才偶尔重新审视我们最开始的决定。

我们指导意见假设读者已经完成了如下的准备工作。

- 已经完成了足够多的问题制定、数据清理等基本工作。这样我们接下来在模型架构和训练配置上花时间才是有意义的。
- 已经有了一个进行训练和评估的流程，而且很容易适配各种感兴趣的模型，并执行训练和预测工作。
- 已经选择和实现了适当的评价指标。这些指标应该尽可能地代表在部署环境中的测量结果。

### 选择模型架构

***总结:*** *当开始一个新项目时，尽量重复使用一个已经有效工作的模型。*

- 选择一个成熟的、常用的模型架构，先让它工作起来。以后总是可以建立一个自定义的模型。
- 模型架构通常有各种超参数来决定模型的大小和其他细节（如层数、层宽、激活函数的类型）。
    - 因此，选择架构实际上意味着选择一个不同的模型系列（每个模型超参数的设置都有一个）。
    - 我们将在[选择初始配置](#choosing-the-initial-configuration)和[科学提升模型性能](#a-scientific-approach-to-improving-model-performance)中考虑选择模型超参数的问题。
- 在可能的情况下，尽量找一篇处理流程尽可能接近手头问题的论文，并以该模型为起点进行复制。

### 选择优化器

***总结:*** *从处理手头问题类型的最流行的优化器开始。*

- 没有哪个优化器是“最好”的。没有优化器能适应所有类型的机器学习问题和模型架构。[甚至比较优化器的性能都是一项困难的任务](https://arxiv.org/abs/1910.05446)。
    🤖
- 我们建议坚持使用成熟的、流行的优化器，特别是在开始新项目时。
    - 理想情况下，选择最流行的优化器用于相同类型的问题。
- 准备好要关注这个优化器的***所有***超参。
    - 具有更多超参数的优化器可能需要更多的调参工作才能找到最佳配置。
    - 这在项目的开始阶段尤其重要，因为我们正试图找到其他各种超参数（如架构超参数）的最佳值，优化器超参数则被视为[干扰参数](#identifying-scientific-nuisance-and-fixed-hyperparameters)。
    - 在项目的初始阶段，最好从一个更简单的优化器开始（例如，具有固定动量的SGD或具有固定$\epsilon$, $\beta_{1}$ 和 $\beta_{2}$ 的Adam），之后再改用更通用的优化器。
-   成熟的优化器包括（但不限于）:
    -   [带动量的SGD](#what-are-the-update-rules-for-all-the-popular-optimization-algorithms)
        (我们推荐Nesterov形式)
    -   [Adam 和 NAdam](#what-are-the-update-rules-for-all-the-popular-optimization-algorithms),
        这比有动量的SGD更通用。但是请注意，Adam有4个可调整的超参数
        [，而且它们都很重要](https://arxiv.org/abs/1910.05446)!
        -   参考
            [应该如何调整Adam的超参数？](#应该如何调整Adam的超参数？)

### 选择批量大小

***总结:*** *批量大小控制着训练速度，不应该被用来直接调整验证集的性能。通常情况下，理想的批量大小是可用硬件所支持的最大批量大小。*

-   批量大小是决定*训练时间*和*计算资源消耗*的一个关键因素。
-   增加批量大小通常会减少训练时间。这可能是非常有益的，因为它：  
    - 允许在一个固定的时间间隔内对超参数进行更彻底的调整，可能会产生更好的最终模型。    
    - 减少开发周期的延迟，允许更频繁地测试新想法。
-   增加批量大小可能会减少、增加或不改变资源消耗。（译者注：详见[选择批量大小以尽量减少资源消耗](#选择批量大小以尽量减少资源消耗)）
-   批量大小*不应该*被当作验证集性能的可调整超参数。
    -   只要所有的超参数都调整得很好（尤其是学习率和正则化超参数），并且训练步骤的数量足够多，那么使用任何批量大小都可以达到相同的最终性能。 (参考
        [Shallue et al. 2018](https://arxiv.org/abs/1811.03600))。
    -   请参考 [为什么不应该调整批量大小来直接改善验证集性能？](#为什么不应该调整批量大小来直接改善验证集性能？) （译者注：这也不尽正确，因为调整批量大小可能会导致采样的分布产生变化，也即每一次参数更新所参考的样本分布。对于某些数据分布敏感的任务，例如无监督学习，或者数据集过小或者噪声很多，则批量大小可能会影响最终性能，批量大小的选择应该参考前人工作和自己尝试确定。）
...
- 对于一个给定的模型和优化器，通常会有一个可用硬件（如GPU）支持的批量大小范围。限制性因素通常是硬件加速器的内存（如GPU内存）。
- 不幸的是，如果不运行或至少不编译完整的训练程序，就很难计算出哪种批量大小适合内存。
- 最简单的解决方案通常是以不同的批量大小（例如增加2的幂）运行训练作业，进行少量的步骤，直到其中一个作业超过了可用的内存。
- 对于每个批次的大小，我们应该训练足够长的时间，以获得对*训练吞吐量*的可靠估计。

<p align="center">训练吞吐量 = (# 每秒处理的样本数)</p>

<p align="center">或者等效于 <em>每步的时间</em>。</p>

<p align="center">每步的时间 = (批量大小) / (训练吞吐量)</p>

- 当加速器（如GPU）还没有饱和时，如果批处理量增加一倍，训练吞吐量也应该增加一倍（或者至少接近一倍）。等价地，每一步的时间应该随着批处理量的增加而保持不变（或至少接近不变）。
- 如果不是这样，那么训练管道就有一个瓶颈，如I/O或计算节点之间的同步。这可能值得在继续进行之前进行诊断和纠正。

- 如果训练吞吐量只增加到某个最大的批处理量，那么我们应该只考虑到那个最大的批处理量，即使硬件支持更大的批处理量。    
    - 使用更大的批处理规模的所有好处都是假设训练吞吐量增加。如果没有，那就解决瓶颈问题或使用较小的批处理量。   
    - **梯度累积**模拟了一个比硬件所能支持的更大的批处理量，因此不提供任何吞吐量的好处。在应用工作中一般应避免使用。
- 每次改变模型或优化器时，可能需要重复这些步骤（例如，不同的模型结构可能允许更大的批量大小以适应内存）。
...
-   我们通常可以认为每一步的时间对于所有可行的批量大小来说都是近似恒定的。当没有并行计算的开销，并且所有的训练瓶颈都已经被诊断和纠正时，这是正确的（关于如何识别训练瓶颈，请参见[上一节](#确定可行的批量大小并估计训练吞吐量)）。在实践中，增加批处理规模通常至少会有一些额外开销。
-   随着批处理量的增加，达到固定性能目标所需的总步骤数通常会减少（只要在改变批处理量时对所有相关的超参数（如学习率）进行重新调整。 [Shallue et al. 2018](https://arxiv.org/abs/1811.03600)）。（译者注：这一节所提到的总步数/总时间都是以达到某个性能（收敛）为标准，而不是以经历固定轮次（epoch）为标准）
    - 例如，增加一倍的批处理量可能使所需的总步骤数减少一半。这被称为**完美的缩放**。    
    - 完美缩放对所有批量大小都适用，直到一个临界批量大小，超过这个临界批量大小，就会获得递减的回报。   
    - 最终，增加批量大小不再减少达到固定性能目标的所需训练步骤的数量（但绝不会增加）。
-   因此，使训练时间最小化的批量大小，通常是仍能减少所需训练步骤数量的最大批量大小。（译者注：一般而言，批量越大，并行性越好，总时间越少）
    -   这个批量大小取决于数据集、模型和优化器，除了对每一个新问题进行实验发现之外，如何计算它是一个公开的问题。🤖
    -   在比较批量大小时，要注意每个样本的预算/[轮次（epoch）的](https://developers.google.com/machine-learning/glossary#epoch)预算（在固定训练样本遍历次数的情况下运行所有实验）和步骤的预算（在固定训练步骤的情况下运行所有实验）之间的区别。
        -   用一个轮次（epoch）预算来比较批量大小，只能探测到“完美的缩放”，即使更大的批量大小可能仍然通过减少所需的训练步骤来提供有意义的加速。（译者注：也就是使用更大的批量大小可能更快收敛，即便轮次（epoch）数量可能增加）
    -   通常情况下，可用硬件支持的最大批量大小将小于临界批量大小。因此，一个好的经验法则（没有进行任何实验）是使用尽可能大的批处理量。
- 如果使用较大的批处理量，最终会增加总的训练时间，那就没有意义了。
